...
class TestFaissVectorStore(unittest.TestCase):
    ...
    def test_documents(self):
        ...
        self.sample_data["firstdate_begin"] = pd.to_datetime(self.sample_data["firstdate_begin"])
        before_one_year_df = self.sample_data[self.sample_data['firstdate_begin'] < date_limite]

        self.assertEqual(len(before_one_year_df), 0, "Il y a des données de plus d'un ans")
